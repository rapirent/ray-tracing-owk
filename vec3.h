...
class vec3 {
    public:
        double e[3];
        vec3() : e{0,0,0} {} // initilize like e = {0, 0, 0}
...
/*
 * to have a uniform scatter direction for all angles away from the hit point,
...
inline vec3 refract(const vec3& u, const vec3& n, double etai_over_etat) {
    //R_perp' = R' *sin(theta) = (R+n*cos(theta)) * (eta/eta') ~= (R + (-R*n)*n) * (eta/eta')
    //R = u, R' = v
    auto cos_theta = fmin(dot(-u, n), 1.0); // cmath: return the mininal one
